...
class Con_PosFold6(nn.Module):
    #merge method: concate
    def __init__(self, in_shape, out_shape, use_const=False,i=None):
        super().__init__()
        ...
        if i==0:
            ...
            self.conv1 = ConvLayer(out_c*2, in_c*in_h*in_w//out_h//out_w, 1)
        elif i==2:
            self.conv1 = ConvLayer(out_c+512, in_c*in_h*in_w//out_h//out_w, 1)
        self.num_fold = int(math.log(out_h, 2)) - int(math.log(in_h, 2))
        self.conv_merge = ConvLayer(
            2*in_c*in_h*in_w//out_h//out_w*(4**self.num_fold), 
            in_c*in_h*in_w//out_h//out_w*(4**self.num_fold), 
            1,
        )
        
        self.folder = Fold()

    def forward(self, emb, hidden_feat, attn_feat_Map=None, is_first=False):
        if is_first:
            emb = attn_feat_Map
            emb = self.conv0(emb)
        out = self.conv1(emb)
