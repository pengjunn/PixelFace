...
from bird_dataset import prepare_data
...
def train(dataloader, cnn_model, rnn_model, batch_size,
	...
	for step, data in enumerate(dataloader, 0):
		...
		if step % UPDATE_INTERVAL == 0:
			...
			writer.add_scalar(f'train_s_loss0', float(s_cur_loss0), count)
			writer.add_scalar(f'train_s_loss1', float(s_cur_loss1), count)
			writer.add_scalar(f'train_w_loss0', float(w_cur_loss0), count)
			writer.add_scalar(f'train_w_loss1', float(w_cur_loss1), count)
			...
			if len(dataloader_val) > 0:
				...
				print('| end epoch {:3d} | '
					  ...
					  .format(epoch, lr,
							  s_loss, w_loss,
							  best_loss, best_ep))
				writer.add_scalar(f'val_s_loss', float(s_loss), epoch)
				writer.add_scalar(f'val_w_loss', float(w_loss), epoch)
				writer.add_scalar(f'val_loss', float(s_loss + w_loss), epoch)
